-- Fix: hidden sessions should still list participants for staff (coach/manager).
-- The previous check returned no rows for *everyone* on hidden sessions, so the home
-- "Participants (next)" block stayed empty while calendar counts still included the session.
...
begin
  ...
  -- Hidden sessions: block athlete browse only; staff may list participants.
