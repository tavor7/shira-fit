-- Exclude Super-User-hidden (session, athlete) pairs from the accounting/
-- attendance numbers that manager_weekly_stats_core computes for app-account
-- registrations, so a hidden workout truly stops counting as debt/usage for
-- that athlete. Manual (walk-in) participant branches are untouched — hiding
-- only applies to app-account athletes. Capacity/utilization, coach payout,
-- waitlist count, and the missing-attendance staff nudge list are
-- deliberately left unfiltered: a hidden athlete still occupies a real seat,
-- the coach is still paid for a real session, and staff still need to manage
-- the hidden athlete's attendance normally.
--
-- Function body is copied wholesale from supabase/migrations/20260814120000_
-- session_custom_coach_rate.sql (the current definition) with
-- `and not public.is_registration_hidden(...)` predicates added to every
-- app-registration branch that feeds athlete debt/attendance/payment totals.
...
begin
  ...
  where s.session_date between v_start and v_end
    and not public.is_registration_hidden(c.session_id, c.user_id);
  ...
  where s.session_date between v_start and v_end
    ...
    and not public.is_registration_hidden(reg.session_id, reg.user_id);
