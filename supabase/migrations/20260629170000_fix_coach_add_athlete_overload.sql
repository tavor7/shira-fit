-- Remove legacy 2-arg overloads that conflict with coach_add_athlete(uuid, uuid, boolean).
-- PostgREST cannot pick a candidate when only p_session_id + p_user_id are sent.
