-- List payments without receipts + bulk create receipts linked to existing payments.

create or replace function public._document_source_for_payment_row(
  p_row_kind text,
  p_record_id uuid,
  out p_source_type public.document_source_type,
  out p_source_id uuid
)
returns record
language plpgsql
immutable
as $$
begin
  p_source_id := p_record_id;
  case p_row_kind
    when 'account' then p_source_type := 'account_payment';
    when 'session_reg', 'session_manual' then p_source_type := 'session_payment';
    when 'cancellation' then p_source_type := 'cancellation_penalty';
    else
...
declare
  ...
  v_source_id uuid;
begin
  select src.p_source_type, src.p_source_id
  into v_source, v_source_id
  from public._document_source_for_payment_row(p_row_kind, p_record_id) as src;

  return exists (
    select 1
    from public.documents d
    where d.source_type = v_source
...
begin
  ...
  enriched as (
    select
      ...
      p.created_at,
      p.coach_name,
      public._service_type_from_session(p.max_participants, p.is_kickbox) as service_type,
  ...
  select
    coalesce((select json_agg(row_to_json(p) order by p.paid_at desc, p.created_at desc, p.row_id desc) from page p), '[]'::json),
    t.total_count,
    t.total_amount
...
begin
  ...
  select src.p_source_type, src.p_source_id
  into v_source, v_source_id
  from public._document_source_for_payment_row(v_row.row_kind, v_row.record_id) src;
...
begin
  ...
  if jsonb_array_length(p_row_ids) > 100 then
...
grant execute on function public.staff_list_payments_without_receipt(date, date, int, int) to authenticated;
grant execute on function public.create_documents_from_payments(jsonb) to authenticated;

comment on function public.staff_list_payments_without_receipt(date, date, int, int) is
  'Payments (account, session, cancellation) with no active linked receipt document.';
comment on function public.create_documents_from_payments(jsonb) is
  'Bulk-create receipt documents for existing payments by staff_list_received_payments row_id values.';
