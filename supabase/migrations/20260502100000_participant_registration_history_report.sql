-- Athlete activity report: fix UNION + ORDER BY for RETURN QUERY, add attendance,
-- payment method, and cancellation audit (reason + within-24h late flag).
-- Optional p_athlete_key restricts rows to one profile user id, manual_participants.id,
-- or manual_participants.linked_user_id (fixes linked quick-add vs RPC athlete_user_id).

drop function if exists public.participant_registration_history(date, date, text);
