-- Bugfix: the Super User must see COMPLETE REAL data everywhere, including
-- this dashboard's accounting numbers — not just the roster/reports screens.
-- The Tier-A hide_from_athlete exclusions added in
-- 20260829140000_manager_weekly_stats_hide_filter.sql were unconditional,
-- so even the Super User's own Overview saw reduced counts/totals for a
-- hidden athlete. Every such predicate now bypasses when the caller is the
-- Super User; regular managers still see the accounting-override exclusion
-- as before. Tier-B numbers (capacity/utilization, coach payout, waitlist,
-- missing-attendance nudge) remain untouched — never filtered by hiding.
