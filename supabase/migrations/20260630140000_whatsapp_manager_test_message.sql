-- Manager quick-send WhatsApp test message (bypasses user opt-in).

create or replace function public.send_whatsapp_manager_test_message(
  p_user_id uuid,
  p_template text default 'waitlist_spot'
)
returns json
language plpgsql
security definer
...
declare
  ...
  v_template text := lower(trim(coalesce(p_template, 'waitlist_spot')));
  v_phone text;
  v_name text;
  v_delivery_id uuid;
  v_dedupe text;
...
grant execute on function public.send_whatsapp_manager_test_message(uuid, text) to authenticated;
