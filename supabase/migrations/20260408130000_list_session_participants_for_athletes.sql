-- Allow athletes to view participant names for a session detail screen.
-- This is implemented as a security definer RPC so RLS on profiles/manual_participants doesn't block.
...
begin
  ...
  -- Only approved athletes (or staff) can view participant names.
  if not (public.is_coach_or_manager(v_uid) or exists (
  ...
  -- Manual participants are staff-only in RLS; expose their names only to staff.
  if public.is_coach_or_manager(v_uid) then
    return query
