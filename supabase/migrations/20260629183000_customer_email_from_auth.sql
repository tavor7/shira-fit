-- Resolve customer email from auth.users (profiles do not store email).

create or replace function public._auth_user_email(p_user_id uuid)
returns text
language sql
stable
security definer
...
create or replace function public._resolve_customer_email(
...
as $$
  select coalesce(
    ...
    public._auth_user_email(p_profile_user_id)
  );
$$;
...
declare
  v_id uuid;
  v_email text := public._resolve_customer_email(p_profile_user_id, p_email);
begin
...
-- Restore correct payment-row kinds + snapshot address/email from auth.
...
begin
  ...
  select c.profile_user_id, coalesce(c.email, v_doc.customer_email)
  into v_profile_user_id, v_customer_email
  from public.customers c
  where c.id = v_doc.customer_id;

  v_customer_email := public._resolve_customer_email(v_profile_user_id, v_customer_email);
  ...
  if v_profile_user_id is not null and v_customer_email is not null then
    update public.customers
    set email = v_customer_email, updated_at = now()
...
-- create_document_with_payment: snapshot resolved auth email on the document row.
