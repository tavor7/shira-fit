-- hello_world test template + synchronous delivery status for manager test sends.
...
begin
  ...
  loop
    ...
    exit when v_status in ('sent', 'failed', 'skipped') or v_attempt >= 12;
  end loop;
  ...
  return json_build_object(
    'ok', false,
    'error', coalesce(v_error, v_skip, 'delivery_failed'),
    'delivery_id', v_delivery_id,
    'user_name', v_name,
    'phone', v_phone,
    'template', v_template,
