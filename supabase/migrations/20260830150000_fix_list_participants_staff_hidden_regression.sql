-- Fix a regression introduced in 20260829130000_list_session_participants_hide_filter.sql:
-- the whole-session `is_hidden` (browse-visibility) early-return had lost its
-- `and not v_is_staff` guard, so coaches/managers got an EMPTY roster for any
-- session flagged is_hidden=true instead of the full unfiltered list. Restores
-- the original behavior (from 20260629188000_list_session_participants_athlete_manual.sql)
-- while keeping this migration's per-audience hide-scope filtering intact.
