-- Staff roster edits after session end + per-session custom slot price.
-- Recreates coach_add_athlete, add_manual_participant_to_session (no session_ended block).
-- Adds training_sessions.custom_slot_price_ils and session_billing_price_ils.
-- Updates manager_weekly_stats to honor custom session pricing.

alter table public.training_sessions
  add column if not exists custom_slot_price_ils numeric(12, 2) null
  check (custom_slot_price_ils is null or custom_slot_price_ils >= 0);

comment on column public.training_sessions.custom_slot_price_ils is
  'One-time per-session price (ILS) for every billing slot on this workout; overrides tier and per-athlete pricing.';
...
as $$
  select coalesce(
    (select ts.custom_slot_price_ils from public.training_sessions ts where ts.id = p_session_id),
    case
      when p_user_id is not null then
        public.effective_session_price_ils(
          p_user_id,
          (select ts.max_participants from public.training_sessions ts where ts.id = p_session_id limit 1)
        )
      else
        (
          select scp.price_ils
          from public.session_capacity_pricing scp
          join public.training_sessions ts on ts.id = p_session_id and scp.max_participants = ts.max_participants
          limit 1
        )
    end,
    0::numeric
...
comment on function public.session_billing_price_ils(uuid, uuid) is
  'Billing price for a session slot: custom_slot_price_ils when set, else athlete override or capacity tier.';

grant execute on function public.session_billing_price_ils(uuid, uuid) to authenticated;

create or replace function public.staff_set_session_custom_slot_price(
  p_session_id uuid,
  p_price_ils numeric
)
returns json
...
begin
  ...
  set custom_slot_price_ils = case when p_price_ils is null then null else round(p_price_ils::numeric, 2) end
...
grant execute on function public.staff_set_session_custom_slot_price(uuid, numeric) to authenticated;
...
begin
  ...
  from (
    ...
    arrived_money as (
      ...
      union all
      select
        coalesce(public.session_billing_price_ils(s.id, null), 0)::numeric,
  ...
  from (
    with expected_slot_money as (
      ...
      where s.session_date between v_start and v_end
        ...
        and reg.attended is true
      union all
      select m.session_id, coalesce(public.session_billing_price_ils(s.id, null), 0)::numeric
      from public.session_manual_participants m
      join public.training_sessions s on s.id = m.session_id
      where s.session_date between v_start and v_end
        and m.attended is true
