-- Expected daily breakdown + account payments grouped in collections.

-- Overview: daily revenue breakdown + ended sessions missing attendance marks.
...
begin
  ...
  from (
    with expected_slot_money as (
      select reg.session_id, coalesce(scp.price_ils, 0)::numeric as exp_amt
      from public.session_registrations reg
      join public.training_sessions s on s.id = reg.session_id
      left join public.session_capacity_pricing scp on scp.max_participants = s.max_participants
      where s.session_date between v_start and v_end
        ...
        and reg.attended is true
      union all
      select m.session_id, coalesce(scp.price_ils, 0)::numeric
      from public.session_manual_participants m
      join public.training_sessions s on s.id = m.session_id
      left join public.session_capacity_pricing scp on scp.max_participants = s.max_participants
      where s.session_date between v_start and v_end
        and m.attended is true
      union all
      select reg.session_id, coalesce(scp.price_ils, 0)::numeric
      from public.session_registrations reg
      join public.training_sessions s on s.id = reg.session_id
      left join public.session_capacity_pricing scp on scp.max_participants = s.max_participants
      where s.session_date between v_start and v_end
      ...
      where s.session_date between v_start and v_end
        ...
        and m.charge_no_show is true
      union all
      select c.session_id, coalesce(scp.price_ils, 0)::numeric
      from public.cancellations c
      join public.training_sessions s on s.id = c.session_id
      left join public.session_capacity_pricing scp on scp.max_participants = s.max_participants
      where s.session_date between v_start and v_end
