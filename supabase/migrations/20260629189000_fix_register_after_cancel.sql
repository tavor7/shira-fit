-- Re-register after self-cancel: cancelled rows share (session_id, user_id) with the unique index.
-- A plain INSERT (or ON CONFLICT … WHERE that does not match) can still surface already_registered.
