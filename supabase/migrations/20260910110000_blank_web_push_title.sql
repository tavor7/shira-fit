-- iOS always prefixes an installed web app's push notifications with "from Shira Fit"
-- itself (a system-level attribution line we cannot suppress), so sending our own title
-- of "Shira Fit" on top of that just duplicates it. Blank the title for the web-push
-- channel only (invoke_send_web_push_edge calls) - native Expo push notifications don't
-- have this duplication problem (no OS-injected "from" line there) so their title stays
-- "Shira Fit" unchanged.
...
begin
  ...
  if v_token is not null and length(trim(v_token)) > 0 then
    begin
      perform net.http_post(
        url := 'https://exp.host/--/api/v2/push/send',
        headers := jsonb_build_object('Content-Type', 'application/json'),
        body := jsonb_build_object('to', v_token, 'title', 'Shira Fit', 'body', v_body)
      );
    exception when others then
      null;
    end;
  ...
  begin
    perform public.invoke_send_web_push_edge(v_uid, '', v_body, '{}'::jsonb);
  exception when others then
    null;
  end;
...
begin
  ...
  loop
    if v_row.expo_push_token is not null and length(trim(v_row.expo_push_token)) > 0 then
      begin
        perform net.http_post(
          url := 'https://exp.host/--/api/v2/push/send',
          headers := jsonb_build_object('Content-Type', 'application/json'),
          body := jsonb_build_object('to', v_row.expo_push_token, 'title', 'Shira Fit', 'body', v_body)
        );
      exception when others then
        null;
      end;
    ...
    begin
      perform public.invoke_send_web_push_edge(v_row.user_id, '', v_body, '{}'::jsonb);
    exception when others then
      null;
    end;
