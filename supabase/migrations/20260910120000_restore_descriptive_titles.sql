-- An empty title (previous migration) broke delivery entirely on iOS - notifications with
-- no title apparently get silently dropped rather than just missing the "from Shira Fit"
-- duplication issue we were trying to fix. Restoring a real title for both channels
-- (native Expo push and web push), but a short one describing what the message actually
-- is about, instead of the generic "Shira Fit" repeated on every single notification.
