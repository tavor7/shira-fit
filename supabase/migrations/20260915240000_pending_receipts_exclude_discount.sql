-- Discount account payments aren't real money received, so they must never appear in the
-- "pending receipts" list (staff_list_payments_without_receipt) — no Israeli tax receipt
-- should ever be proposed/issued for a discount. There is no trigger auto-creating
-- documents from athlete_account_payments (verified: only a validation trigger and
-- activity-log triggers exist on that table), so excluding discount here, in the one
-- place pending-receipt candidates are enumerated, fully closes this off — staff can
-- never be prompted to issue a receipt for a discount.
--
-- Full function body otherwise byte-for-byte identical to the live definition; only the
-- `account` branch of the `unified` CTE gained a payment_method filter.
