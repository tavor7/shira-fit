...
import { formatISODateFull, formatISODateFullWithWeekdayAfter } from "./dateFormat";
...
export type HomePriorityLabelSegment = { text: string; dir: "ltr" | "rtl" };
...
export async function fetchStaffLateCancellationItems(
  ...
  const rows: { item: HomePriorityAlertItem; t: number }[] = [];
  for (const row of data as {
    ...
    const dayStr = formatISODateFullWithWeekdayAfter(sess.session_date, language);
    ...
    rows.push({
      t: sessionStartsAt(sess.session_date, sess.start_time).getTime(),
      item: {
        id: `lc-${row.id}`,
        label: flatLabel,
        labelSegments: [
          { text: lead, dir: leadDir },
          { text: dayTime, dir: dayTimeDir },
          { text: beforeName, dir: beforeNameDir },
          { text: name, dir: nameDir },
        ],
