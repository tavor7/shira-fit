...
import { formatIls, autoGenerateDocumentPdfIfReady, fetchDocumentCustomerEmail, invokeSendDocumentEmail } from "../lib/documents";
  ...
  async function createSelected() {
    ...
    try {
      ...
      for (const doc of result.created) {
        ...
        if (emailCustomers) {
          try {
            const customerEmail = await fetchDocumentCustomerEmail(doc.document_id);
            if (customerEmail) {
              await invokeSendDocumentEmail(doc.document_id, customerEmail);
              emailCount += 1;
            }
          } catch {
            /* skip failed sends */
      ...
      const msg =
        language === "he"
          ? `נוצרו ${result.created_count} קבלות${result.failed_count ? ` · ${result.failed_count} נכשלו` : ""}${generatePdfs && pdfCount ? ` · ${pdfCount} PDF` : ""}${emailCustomers && emailCount ? ` · ${emailCount} נשלחו` : ""}`
          : `${result.created_count} receipt(s) created${result.failed_count ? ` · ${result.failed_count} failed` : ""}${generatePdfs && pdfCount ? ` · ${pdfCount} PDF` : ""}${emailCustomers && emailCount ? ` · ${emailCount} emailed` : ""}`;
