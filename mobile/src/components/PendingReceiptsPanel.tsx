...
export function PendingReceiptsPanel({ enabled, header, onCreated, testingMode = false }: Props) {
  ...
  const createProgress = bulkJob?.key === "pendingReceipts.create" ? bulkJob : null;
  const creating = createProgress !== null;
  const createFetchingRef = useRef(false);
  ...
  async function createSelected() {
    if (selectedCount === 0 || bulkJob || createFetchingRef.current) return;
    createFetchingRef.current = true;
    let result: Awaited<ReturnType<typeof createDocumentsFromPayments>>;
    try {
      result = await createDocumentsFromPayments([...selected]);
    } catch (e) {
      createFetchingRef.current = false;
      showToast({
    ...
    // (and stays visible) even if the manager navigates to another screen mid-run. Cancelling
    // only skips remaining PDF/email steps — receipts already created here stay created.
    await runBulkJob("pendingReceipts.create", result.created.length, async (setDone, isCancelled) => {
      let pdfCount = 0;
      let emailCount = 0;
      ...
      if (!cancelled && emailCustomers && createdDocIds.length > 0) {
        try {
      ...
      const msg =
        language === "he"
          ...
          : `${result.created_count} receipt(s) created${result.failed_count ? ` · ${result.failed_count} failed` : ""}${generatePdfs && pdfCount ? ` · ${pdfCount} PDF` : ""}${emailCustomers && emailCount ? ` · ${emailCount} customer email(s)` : ""}${cancelledSuffix}`;
      showToast({ message: msg, variant: result.failed_count ? "info" : "success" });
    });
  }

  function cancelCreateSelected() {
